...
std::array<std::string, 9> Result::columns_ = {"subject_id", "beverage_id", "target_bac", "target_bac_time", 
                                    "amount_grams", "amount_beverage", "alcvol", "actual_bac", "actual_bac_time"};
...
Result::Result(int pk, int subject_id, int beverage_id,
                    ...
                    amount_beverage_(amount_beverage), alcvol_(alcvol), actual_bac_(actual_bac), actual_bac_time_(actual_bac_time) {};
...
Result::Result(char** fields) : Model(std::stoi(fields[0])), 
                    ...
                    amount_grams_(atof(fields[5])), amount_beverage_(atof(fields[6])), alcvol_(atof(fields[7])),
                    actual_bac_(atof(fields[8])), actual_bac_time_(Database::ToStdString(fields[9])) {};
...
std::vector<std::string> Result::Serialise() {
    ...
    // Alcohol volume
    returnVec.push_back(std::to_string(this->alcvol_));
    // Actual recorded BAC
