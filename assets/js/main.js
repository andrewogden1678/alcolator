...
let vm = new Vue({
    ...
    methods: {
        ...
        newSubject: function () {
            ...
            let record = this.records.some(r => r.subject_code === this.draft.subject_code);
            if (record) {
                // Dupe found, validation failed
                this.validated.newSubjectCode = false
                return;
            } else {
                // All good
            ...
            // Set default drink to vodka
            let defaultDrink = this.beverages.filter(b => b.name.toLowerCase() == "vodka");
            this.draft.beverage_name = defaultDrink[0].name;
            this.draft.beverage_concentration = defaultDrink[0].concentration;
